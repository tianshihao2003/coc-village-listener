...
1. `Shizuku.unbindUserService(args, conn)` 实际为**三参** `unbindUserService(args, conn, true)`（javap 实测 13.1.5 签名）；
2. AIDL `IClipboardUserService` 不含 `version()`（计划 UserService 代码中的 override 已删）；
3. `NotificationManager.notify(String, Notification)` 单参重载不存在，改用 `notify(tag, tag.hashCode(), notification)`；
4. AppPickerActivity 的 inflate 结果需 `as TextView` 强转；
5. `gradle wrapper` 在境内网络下用 `--gradle-distribution-url` 指向腾讯镜像生成（gradle-wrapper.properties 中的 distributionUrl 为镜像地址，GitHub Actions 境外亦可正常下载）；
6. `local.properties` 的 sdk.dir 必须用正斜杠写法 `E:/android-tools/android-sdk`（properties 反斜杠转义陷阱）。
